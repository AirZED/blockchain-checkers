...
describe("blockchain-checkers", () => {
  ...
  before(async () => {
    ...
    await provider.connection.confirmTransaction(
      await provider.connection.requestAirdrop(host.publicKey, 10 * anchor.web3.LAMPORTS_PER_SOL)
    );
    await provider.connection.confirmTransaction(
      await provider.connection.requestAirdrop(player1.publicKey, anchor.web3.LAMPORTS_PER_SOL)
    );
    await provider.connection.confirmTransaction(
      await provider.connection.requestAirdrop(player2.publicKey, anchor.web3.LAMPORTS_PER_SOL)
    );
    await provider.connection.confirmTransaction(
      await provider.connection.requestAirdrop(player3.publicKey, anchor.web3.LAMPORTS_PER_SOL)
    );
    await provider.connection.confirmTransaction(
      await provider.connection.requestAirdrop(player4.publicKey, anchor.web3.LAMPORTS_PER_SOL)
    );
  ...
  it("Should initialize a tournament", async () => {
    ...
    expect(tournamentAccount.host.toString()).to.equal(host.publicKey.toString());
  ...
  it("Should allow player 1 to join the tournament", async () => {
    ...
    const tournamentAccount = await program.account.tournament.fetch(tournamentPDA);
    expect(tournamentAccount.players).to.include.deep.members([player1.publicKey]);
  });
  ...
  it("Should allow player 2 to join the tournament", async () => {
    ...
    // Verify player was added
    const tournamentAccount = await program.account.tournament.fetch(tournamentPDA);
    expect(tournamentAccount.players).to.include.deep.members([player1.publicKey, player2.publicKey]);
  });
  ...
  it("Should allow winners to claim rewards", async () => {
    ...
    const beforeBalance = (await provider.connection.getTokenAccountBalance(winnerAta)).value.amount;
    ...
    const afterBalance = (await provider.connection.getTokenAccountBalance(winnerAta)).value.amount;
